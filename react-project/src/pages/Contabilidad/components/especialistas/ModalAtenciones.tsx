...
import { X, Download, ChevronLeft, ChevronRight, ClipboardList } from 'lucide-react';
import type { EspecialistaAtencion, EspecialistaAtencionesResponse, EspecialistaResumen } from '../../../../services/contabilidad/contaTypes';
import { moneyPEN } from '../../../../utils/money';
import { exportAtenciones, type EspExcelFiltros } from './excelEspecialistas';
...
interface Props {
  ...
  loadPage: (medicoId: number, pagina: number) => Promise<EspecialistaAtencionesResponse>;
  // Dataset completo (tamanio=0) para el export; el server capea a 20k -> 400 {message} legible.
  loadAll: (medicoId: number) => Promise<EspecialistaAtencion[]>;
  onClose: () => void;
}

const ModalAtenciones: React.FC<Props> = ({ medico, filtrosExport, loadPage, loadAll, onClose }) => {
  const [pagina, setPagina] = useState(1);
  const [filas, setFilas] = useState<EspecialistaAtencion[]>([]);
  ...
  useEffect(() => {
    ...
    loadPage(medico.MedicoId, pagina)
      ...
      .catch((e) => { if (!alive) return; setError(e instanceof Error ? e.message : 'Error al cargar las atenciones'); })
  ...
  const onExport = async () => {
    ...
    try {
      ...
      if (all.length === 0) { toast('No hay atenciones para exportar'); return; }
      exportAtenciones(all, { UserName: medico.UserName, Medico: medico.Medico }, filtrosExport);
    } catch (e) {
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-3 border-b border-slate-200 dark:border-slate-700">
          <div>
            <h3 className="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <ClipboardList className="h-5 w-5 text-emerald-600" /> Atenciones · {medico.Medico}
            </h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800">
                  ...
                  <th className="px-3 py-2">Diagnósticos</th>
                  <th className="px-3 py-2 text-center">¿Referida?</th>
              ...
              <tbody>
                ...
                {!loading && error && <tr><td colSpan={9} className="px-3 py-8 text-center text-rose-500">{error}</td></tr>}
                {!loading && !error && filas.length === 0 && <tr><td colSpan={9} className="px-3 py-8 text-center text-slate-400">Sin atenciones en el rango</td></tr>}
                {!loading && !error && filas.map((r) => (
                  <tr key={r.ServiceId} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    ...
                    <td className="px-3 py-2">{r.Consultorio ?? '—'}</td>
                    <td className="px-3 py-2">{r.Paciente ?? '—'}</td>
                    <td className="px-3 py-2 text-xs">{r.EstadoAtencion}</td>
                    ...
                    <td className="px-3 py-2 max-w-xs truncate" title={r.Diagnosticos ?? ''}>{r.Diagnosticos ?? '—'}</td>
                    <td className="px-3 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${r.Referida ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                        {r.Referida ? 'SÍ' : 'NO'}
                      </span>
                    </td>
                    <td className="px-3 py-2 max-w-xs truncate" title={r.ReferidoPor ?? ''}>{r.ReferidoPor ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
