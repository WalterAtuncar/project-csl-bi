/*
  Refactor: sp_PagoMedicoPorConsultorioCompleto
  Objetivo: Mejorar performance y legibilidad eliminando subconsultas por fila,
  pre-agregando conteos y fechas de pago, y asegurando limpieza de temporales.
  Uso: Ejecutar manualmente en el contexto de la base principal que contiene
  las tablas [service], [protocol], [servicecomponent], [servicespaiddetails], [servicespaid], [systemparameter], [systemuser], [person].
  Nota: La relación con ventas se realiza contra [20505310072].[dbo].[venta] como en el SP original.
*/
...
BEGIN
    ...
    -- Temporales base: componentes por consultorio y rango
    ...
    WHERE pr.i_Consultorio = @i_Consultorio
      ...
      AND sr.v_ComprobantePago IS NOT NULL AND LTRIM(RTRIM(sr.v_ComprobantePago)) <> '';

    -- Pre-agregados por servicio: total componentes, pagados y fecha de pago
    ...
    -- Servicios únicos con datos de ventas
    ...
    /*
      Resultset 1: Cabecera por médico
    */
    SELECT
    ...
    /*
      Resultset 2: Detalle por servicio
    */
    SELECT
