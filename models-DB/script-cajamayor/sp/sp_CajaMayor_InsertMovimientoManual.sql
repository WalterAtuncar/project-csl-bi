/*
  SP: sp_CajaMayor_InsertMovimientoManual
  Descripción: Inserta un movimiento manual (I/E) en cajamayor_movimiento.
  Política: En producción usar COMMIT por defecto; ROLLBACK solo para pruebas.
*/

SET ANSI_NULLS ON;
SET QUOTED_IDENTIFIER ON;
GO
IF OBJECT_ID('dbo.sp_CajaMayor_InsertMovimientoManual','P') IS NOT NULL
    DROP PROCEDURE dbo.sp_CajaMayor_InsertMovimientoManual;
GO
CREATE PROCEDURE dbo.sp_CajaMayor_InsertMovimientoManual
    @IdCajaMayorCierre INT,
    @IdTipoCaja        INT,
    @TipoMovimiento    NCHAR(1), -- 'I' o 'E'
    @ConceptoMovimiento NVARCHAR(350) = NULL,
    @Subtotal          DECIMAL(18,2) = NULL,
    @IGV               DECIMAL(18,2) = NULL,
    @Total             DECIMAL(18,2),
    @FechaRegistro     DATETIME,
    @Observaciones     NVARCHAR(200) = NULL,
    @Origen            NVARCHAR(30) = N'manual',
    @CodigoDocumento   NVARCHAR(30) = NULL,
    @SerieDocumento    NVARCHAR(20) = NULL,
    @NumeroDocumento   NVARCHAR(20) = NULL,
    @IdVenta           NVARCHAR(50) = NULL,
    @InsertaIdUsuario  INT
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;

    BEGIN TRY
        BEGIN TRAN;

        INSERT INTO dbo.cajamayor_movimiento (
            i_IdCajaMayorCierre,
            i_IdTipoCaja,
            v_TipoMovimiento,
            v_ConceptoMovimiento,
            d_Subtotal,
            d_IGV,
            d_Total,
            t_FechaMovimiento,
            v_Observaciones,
            v_Origen,
            v_CodigoDocumento,
            v_SerieDocumento,
            v_NumeroDocumento,
            v_IdVenta,
            i_InsertaIdUsuario,
            t_InsertaFecha
        ) VALUES (
            @IdCajaMayorCierre,
            @IdTipoCaja,
            @TipoMovimiento,
            @ConceptoMovimiento,
            @Subtotal,
            @IGV,
            @Total,
            @FechaRegistro,
            @Observaciones,
            @Origen,
            @CodigoDocumento,
            @SerieDocumento,
            @NumeroDocumento,
            @IdVenta,
            @InsertaIdUsuario,
            GETDATE()
        );

        -- Recalcular resumen por tipo
        EXEC dbo.sp_CajaMayor_ResumenTipos @IdCajaMayorCierre = @IdCajaMayorCierre, @ActualizaIdUsuario = @InsertaIdUsuario;

        SELECT TOP (1) *
          FROM dbo.cajamayor_movimiento
         WHERE i_IdCajaMayorCierre = @IdCajaMayorCierre
         ORDER BY i_IdMovimiento DESC;

        COMMIT TRAN;
    END TRY
    BEGIN CATCH
        IF XACT_STATE() <> 0 ROLLBACK TRAN;
        DECLARE @ErrorMessage NVARCHAR(4000) = ERROR_MESSAGE();
        RAISERROR(@ErrorMessage, 16, 1);
    END CATCH
END
GO